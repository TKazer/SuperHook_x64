...
public:
	...
	StatusCode Attach(std::string ProcessName)
	{
		...
		ModuleAddress = reinterpret_cast<DWORD64>(this->GetProcessModuleHandle(ProcessID, ProcessName));
...
static ProcessManager ProcessMgr;

template <typename ReadType>
bool ReadMemory(DWORD64 Address, ReadType& Value)
{
	return ProcessMgr.ReadMemory(Address, Value);
}